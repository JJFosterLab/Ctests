...
#calculate probability density at each angle
dd = with(best_model,
  dmixedvonmises(x = circular(xx,
                            units = 'degrees',
                            rotation = 'clock'
                              ),
                mu1 = q1, #in radians if generated by CircMLE
                kappa1 = k1,
                mu2 = q2, #in radians if generated by CircMLE
                kappa2 = k1,
                prop = lamda#spelled incorrectly in circMLE!
                )
)
with(best_model,
     {
plot(x = xx, #these are angles in degrees
     y = dd, #this is the probability density
     xlim = c(-180, 180), ylim = c(0, max(dd)),
     ylab = 'probability density',
     xlab = 'bearing',
     col = 'salmon', #line colour
     lwd = 2, #line width
     main = paste('von Mises mixture distribution:\n mean1 =',
                  round(deg(q1),1), 
                  'kappa1 =', round(k1,2),
                  '\nmean2 =',
                  round(deg(q2),1), 
